...
'''
This file is from https://github.com/ikostrikov/pytorch-flows 
flows.py
'''
...
class MADE(nn.Module):
    ...
    MADE model: see Figure 1 
    MADE: Autoencoders x Masks, Masks imply a set of connectons is removed s.t., each input is only predicted
    ...
    def __init__(self,
        ...
        # joiner: masked linear layer- 1st fully connected layer
       
        self.joiner = nn.MaskedLinear(num_inputs, num_hidden, input_mask,
                                      num_cond_inputs)

        # trunk: act, linear, act,linear
        # 2nd, 3rd fully connected layer
    ...
    def forward(self, x, cond_inputs=None, mode='direct'):
        '''
        u = T-inverse(x)
        See Masked Autoregressive Flow For Density Estimation  eq(4)
        u_i = (x_i-u_i)exp(-a_i) where u_i = f(x_1:x_{i-1}) a_i = f(x_1:x_{i-1})
        Input: x 
        Output: u= T-inverse(x), log_jacobian of T-inverse = - log_jacobian of T
        '''
        if mode == 'direct':
            # batch_size: bs
            # input_size: d
...
class CouplingLayer(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        ...
        # print(inputs.size(), mask.size(), masked_inputs.size())
