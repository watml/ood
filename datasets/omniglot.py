...
class OMNIGLOT(BaseDataset):
    def __init__(self, path, n_class=30, InD=None):
        ...
        self.train_split = datasets.Omniglot(self.path, download=True,
                                             transform=transforms.Compose(
                                                 [transforms.CenterCrop(84),
                                                  transforms.Resize(28),
                                                  transforms.ToTensor()])
                                             )
        self.test_split = datasets.Omniglot(self.path, download=True,
                                            transform=transforms.Compose(
                                                [transforms.CenterCrop(84),
                                                 transforms.Resize(28),
                                                 transforms.ToTensor()])
    ...
    def __repr__(self):
        return ("OOD detection on OMNIGLOT (InD = {} )").format(self.InD)