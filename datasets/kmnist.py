...
class KMNIST(BaseDataset):
    def __init__(self, path, n_class=10, InD=None):
        ...
        """
        Class constructor.
        :param path: The folder in which to download KMNIST.
        ...
        self.train_split = datasets.KMNIST(self.path, train=True, download=True, transform=transforms.ToTensor())
        self.test_split = datasets.KMNIST(self.path, train=False, download=True, transform=transforms.ToTensor())
    ...
    def __repr__(self):
        return ("OOD detection on KMNIST (InD = {} )").format(self.InD)