...
class FMNIST(BaseDataset):
    def __init__(self, path, n_class = 10, InD = None, train_intra = True):

        # type: (str) -> None

        """
        Class constructor.
        :param path: The folder in which to download FMNIST.
        ...
        self.InD = InD
        self.train_intra = train_intra # for intra-dataset OOD, this argument is not used for inter-dataset OOD.
        
        self.name ='fmnist'

        # Get train and test split
        self.train_split = datasets.FashionMNIST(self.path, train=True, download=True, transform=transforms.ToTensor())
        self.test_split = datasets.FashionMNIST(self.path, train=False, download=True, transform=transforms.ToTensor())
    ...
    def test(self, InD):
        ...
        if len(InD) == 1:
            # treat one class for ind, rest all classes as ood.
            self.InD = int(InD)
            if self.train_intra == True:
                test_idx = [idx for idx in test_idx if self.test_split[idx][1] == self.InD]
            else:
                test_idx = [idx for idx in test_idx if self.test_split[idx][1] != self.InD]
    ...
    def __repr__(self):
        return ("OOD detection on FMNIST (InD = {} )").format(self.InD)