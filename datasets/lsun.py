...
class LSUN(BaseDataset):
    def __init__(self, path, n_class=10, InD=None):
        ...
        self.train_split = datasets.LSUN(self.path, classes='test',
                                         transform=transforms.Compose(
                                             [transforms.CenterCrop(224),
                                              transforms.Resize(32),
                                              transforms.ToTensor()])
                                         )
        self.test_split = datasets.LSUN(self.path, classes='test',
                                        transform=transforms.Compose(
                                            [transforms.CenterCrop(224),
                                             transforms.Resize(32),
                                             transforms.ToTensor()])
    ...
    def __repr__(self):
        return ("OOD detection on LSUN (InD = {} )").format(self.InD)